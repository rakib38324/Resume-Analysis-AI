...
const convertDocToDocx = (inputBuffer) => {
  return new Promise((resolve, reject) => {
    libre.convert(inputBuffer, '.docx', undefined, (err, done) => {
      if (err) return reject(err);
      resolve(done); // returns docx buffer
    });
  });
};
  ...
  try {
    ...
    if (!['pdf', 'doc', 'docx'].includes(ext)) {
      return res.status(400).json({
        error: 'Only PDF, DOC, DOCX files are supported.',
      });
    }
    ...
    // ======================
    // DOC (FIXED FOR WINDOWS)
    // ======================
    else if (ext === 'doc') {
      try {
        const docxBuffer = await convertDocToDocx(buffer);
        extractedText = await extractDOCX(docxBuffer);
      } catch (err) {
        console.error('DOC conversion error:', err.message);
        return res.status(400).json({
          error: 'Failed to convert DOC file. Please re-save as DOCX.',
        });
      }
    }
